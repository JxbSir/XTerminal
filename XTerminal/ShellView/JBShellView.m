...
- (id)initWithFrame:(CGRect)frame prompt:(NSString *)prompt inputHandler:(JBShellViewInputProcessingHandler)inputHandler
{
    ...
    if (self) {
		...
		[self setFont:[NSFont fontWithName:@"Menlo" size:15.0f]];
...
- (void)keyDown:(NSEvent *)theEvent {
    ...
    if ([self selectedRange].location < self.commandStart && !(modifierFlags & NSEventModifierFlagShift && (arrowKey))) {
...
- (BOOL)textView:(NSTextView *)textView doCommandBySelector:(SEL)commandSelector {
	...
	// Unfinished support for a suggestions window
